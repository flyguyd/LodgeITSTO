...
export class HoldSlideoutComponent implements OnInit, OnDestroy {
  ...
  readonly g = signal({ firstName: '', lastName: '', email: '', phone: '' });
  ...
  private load(): void {
    this.api.hold(this.id()).subscribe({
      next: (h) => { this.h.set(h); this.g.set({ firstName: h.guest?.firstName ?? '', lastName: h.guest?.lastName ?? '', email: h.guest?.email ?? '', phone: h.guest?.phone ?? '' }); this.updateTimer(); },
  ...
  setG(k: 'firstName' | 'lastName' | 'email' | 'phone', v: string): void { this.g.set({ ...this.g(), [k]: v }); }
  cancel(): void {
    const h = this.h();
    if (!h) return;
  ...
  convert(): void {
    ...
    this.error.set('');
    const g = this.g();
    this.api.convertHold(h.id, { guest: { firstName: g.firstName.trim(), lastName: g.lastName.trim(), email: g.email.trim() || null, phone: g.phone.trim() || null, country: null } }).subscribe({
